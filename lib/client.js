...
//
// Renders a cost card in the layout's `aside` slot (rightmost column). The
// host half accumulates token usage per (provider, model) from live session
// events and serves /dsh-calculator; this half polls that endpoint and shows
// the current session's spend plus the caller's local-day total across all
// sessions (per model, DeepSeek routes billed; third-party models unbilled).
	...
	factory: (require) => {
		...
		const I18N = {
			zh: {
				...
				unbilled: "（未计费）",
				note: "估算口径：DeepSeek 官方价（缓存命中/未命中输入、输出分档，8/17 起按峰谷时段计费）。输出含推理 token，与普通输出同价。与官方账单可能存在差异。",
			...
			en: {
				...
				unbilled: " (unbilled)",
				note: "Estimated at official DeepSeek rates (cache-hit / cache-miss input, output; peak/off-peak pricing from 8/17). Output includes reasoning tokens at the output rate. May differ from the official bill.",
