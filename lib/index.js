...
/** Beijing is UTC+8 with no DST, so a calendar day starts at a fixed UTC offset. */
const BEIJING_UTC_OFFSET_MS = 8 * 60 * 60 * 1000;

/** Start-of-day (Beijing) for a UTC timestamp, as a UTC ms value. */
function beijingDayStart(ms) {
	const shifted = new Date(ms + BEIJING_UTC_OFFSET_MS);
	shifted.setUTCHours(0, 0, 0, 0);
	return shifted.getTime() - BEIJING_UTC_OFFSET_MS;
}

/** "2026-08-17"-style Beijing date label for a start-of-day UTC ms value. */
function beijingDayLabel(dayStartMs) {
	return new Date(dayStartMs + BEIJING_UTC_OFFSET_MS).toISOString().slice(0, 10);
}
...
function apply(ctx) {
	...
	/**
	 * dayStartMs -> Map<"provider/model", accumulator>. Only the current day is
	 * exposed by the report, so the "全部会话累计" card shows **today's** spend.
	 */
	const globalByDay = /* @__PURE__ */ new Map();
	...
	const record = (sessionId, event) => {
		...
		// Day bucket: deduplicate by message.id so fork-copied events are
		// not double-billed. Buckets are keyed by Beijing start-of-day, so
		// only today's events contribute to the reported day total.
		...
		let dayMap = globalByDay.get(dayStart);
		if (dayMap === void 0) {
			dayMap = /* @__PURE__ */ new Map();
			globalByDay.set(dayStart, dayMap);
		}
	...
	const buildReport = () => {
		...
		const dayMap = globalByDay.get(todayStart);
		...
		return {
			...
			/** Start of the reported day (Beijing) as UTC ms; the day total covers events since then. */
			dayStartMs: todayStart,
			/** "2026-08-17"-style Beijing date of the reported day. */
			dayLabel: beijingDayLabel(todayStart),
